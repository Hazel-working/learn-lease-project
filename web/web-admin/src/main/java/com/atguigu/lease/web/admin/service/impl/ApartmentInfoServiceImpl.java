...
public class ApartmentInfoServiceImpl extends ServiceImpl<ApartmentInfoMapper, ApartmentInfo>
    ...
    @Autowired
    private ApartmentFacilityServiceImpl apartmentFacilityService;
    @Autowired
    ...
    public void saveOrUpdateApartment(ApartmentSubmitVo apartmentSubmitVo) {
        ...
        //先调用父类ServiceImpl的方法插入或更新公寓信息,得到公寓id
        super.saveOrUpdate(apartmentSubmitVo);

        if(isUpdate) {
            //更新公寓图片、配套、标签、杂费列表:先删除,再插入
            //1.删除图片列表
    ...
    public void removeApartmentById(Long id) {
        ...
        if (count > 0) {
            //抛异常，终止删除公寓操作，响应错误信息参数给前端
            throw new LeaseException(ResultCodeEnum.ADMIN_APARTMENT_DELETE_ERROR);
            //运行时异常（未受检异常）与编译时异常（受检异常）：
            //编译时异常必须处理，编译器强制要求显式处理
        }

        //调用父类BaseServiceImpl的方法根据id删除公寓信息
        super.removeById(id);
